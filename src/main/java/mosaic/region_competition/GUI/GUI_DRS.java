...
public class GUI_DRS extends GUI {
    PluginSettingsDRS settingsDrs;
    
    public GUI_DRS(PluginSettingsDRS aSettings, ImagePlus aInputImg, boolean aInRcMode) {
        super("Discrete Region Sampling", aSettings, aInputImg, aInRcMode);
        settingsDrs = aSettings;
    }
    ...
    protected void createParametersDialog() {
        ...
        gd.addNumericField("Burn-in factor [0-1]", settingsDrs.burnInFactor, 4, 8, "");
        gd.addNumericField("Off-boundary probability [0-1]", settingsDrs.offBoundarySampleProbability, 4, 8, "");
        gd.addCheckboxGroup(1, 2, new String[] { "biased proposal", "pair proposal" }, new boolean[] { settingsDrs.useBiasedProposal, settingsDrs.usePairProposal });
        ...
        if (gd.wasOKed()) {
            ...
            // DRS settings
            settingsDrs.burnInFactor = (float)gd.getNextNumber();
            settingsDrs.offBoundarySampleProbability = (float)gd.getNextNumber();
            settingsDrs.useBiasedProposal = gd.getNextBoolean();
            settingsDrs.usePairProposal = gd.getNextBoolean();
