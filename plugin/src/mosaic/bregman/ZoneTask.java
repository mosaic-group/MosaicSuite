    ...
    private void doWork() throws InterruptedException {
        ...
        if (AS.iParameters.noise_model == 0) {
            ...
            for (int z = 0; z < AS.nz; z++) {
                for (int i = iStart; i < iEnd; i++) {
                    for (int j = 0; j < AS.nj; j++) {
                        AS.temp3[0][i][j] = Math.pow(((AS.iParameters.ldata / AS.lreg_) * AS.iParameters.gamma - AS.b1k[0][i][j] - AS.temp2[0][i][j]), 2) + 4
                                * (AS.iParameters.ldata / AS.lreg_) * AS.iParameters.gamma * AS.image[0][i][j];
                    }
                }
            }
            ...
            for (int z = 0; z < AS.nz; z++) {
                for (int i = iStart; i < iEnd; i++) {
                    for (int j = 0; j < AS.nj; j++) {
                        AS.w1k[0][i][j] = 0.5 * (AS.b1k[z][i][j] + AS.temp2[z][i][j] - (AS.iParameters.ldata / AS.lreg_) * AS.iParameters.gamma + Math.sqrt(AS.temp3[z][i][j]));
                    }
                }
            }
        }
