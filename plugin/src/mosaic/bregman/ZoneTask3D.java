    ...
    private void doWork() throws InterruptedException {
        ...
        if (AS.p.noise_model == 0) {
            ...
            for (int z = 0; z < AS.nz; z++) {
                for (int i = iStart; i < iEnd; i++) {
                    for (int j = 0; j < AS.nj; j++) {
                        AS.temp3[AS.l][z][i][j] = Math.pow(((AS.p.ldata / AS.p.lreg_[AS.channel]) * AS.p.gamma - AS.b1k[AS.l][z][i][j] - AS.temp2[AS.l][z][i][j]), 2) + 4
            ...
            for (int z = 0; z < AS.nz; z++) {
                for (int i = iStart; i < iEnd; i++) {
                    for (int j = 0; j < AS.nj; j++) {
                        AS.w1k[AS.l][z][i][j] = 0.5 * (AS.b1k[AS.l][z][i][j] + AS.temp2[AS.l][z][i][j] - (AS.p.ldata / AS.p.lreg_[AS.channel]) * AS.p.gamma + Math.sqrt(AS.temp3[AS.l][z][i][j]));
                    }
                }
            }
        }
        ...
        LocalTools.shrink3D(AS.temp1[AS.l], AS.temp2[AS.l], AS.w2zk[AS.l], AS.temp1[AS.l], AS.temp2[AS.l], AS.w2zk[AS.l], AS.p.gamma, iStart, iEnd);
