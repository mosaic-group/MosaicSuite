...
public class Algorithm {
    ...
    public boolean GenerateData(Img<FloatType> image_psf) {
        ...
        /**
         * Write the label image in a convenient form to the filters output
         * image
         */
    ...
    private boolean MoveCandidates(HashMap<Point, ContourParticle> m_Candidates) {
        ...
        while (vPointIterator.hasNext()) {
            ...
            // / If the move doesn't change topology or is allowed (and
            // registered
        ...
        if (settings.m_AllowFusion) {
            ...
            for (Entry<Point, LabelPair> vCRit : m_CompetingRegionsMap.entrySet()) {
                // Entry<Point, Pair<Integer, Integer>>
                // vCRit=competingRegions[i];
                ...
                if (vCheckedLabels.contains(vLabel1) || vCheckedLabels.contains(vLabel2)) {
                    // do nothing, since this label was already in a
                    // fusion-chain
                }
                else {
        ...
        if (vSplit || vMerge) {
            if (m_EnergyFunctional == EnergyFunctionalType.e_DeconvolutionPC) {
                // TODO: this can be removed when the statistics are updated
                // using
    ...
    private void RebuildCandidateList(HashMap<Point, ContourParticle> aReturnContainer) {
        ...
        for (Entry<Point, ContourParticle> vPointIterator : m_InnerContourContainer.entrySet()) {
            ...
            if (vVal.newlyCreated) {
                // this point was created in the last iteration, don't delete it
                // in this one.
                // if balloon force is set, created pixels tend to disappear
                // again.
                // (balloon force boosts particles toward BG - without balloon
                // force,
                ...
                if (vLabelOfDefender != vLabelOfPropagatingRegion) {
                    ...
                    else // / the point is already part of the candidate list
                    {
                        ...
                        if (!vContourPointItr.hasLabelBeenTested((vLabelOfPropagatingRegion))) {
                            ...
                            EnergyResult energyAndMerge = CalculateEnergyDifferenceForLabel(vNeighborIndex, vContourPointItr, vLabelOfPropagatingRegion);
    ...
    private void ChangeContourPointLabelToCandidateLabel(Entry<Point, ContourParticle> aParticle) {
        ...
        // /
        // / Erase the point from the surface container in case it now belongs
        // to
    ...
    private void RemoveSinglePointRegions() {
        ...
        // TODO: here we go first through contour points to find different
        // labels
        // and then checking for labelInfo.count==1.
        // instead, we could iterate over all labels (fewer labels than contour
        // points),
        // detecting if one with count==1 exists, and only IFF one such label
        // exists searching for the point.
        // but atm, im happy that it detects "orphan"-contourPoints (without
        // attached labelInfo)
    ...
    private void UpdateStatisticsWhenJump(Entry<Point, ContourParticle> aParticle, int aFromLabelIdx, int aToLabelIdx) {
        ...
        // samples:
        double vToLabelSumOfSq = aToLabel.var * (vNTo - 1.0) + vNTo * aToLabel.mean * aToLabel.mean;
        double vFromLabelSumOfSq = aFromLabel.var * (aFromLabel.count - 1.0) + vNFrom * aFromLabel.mean * aFromLabel.mean;
